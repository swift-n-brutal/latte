...
#else
int cuda_num_threads() { return CAFFE_CUDA_NUM_THREADS; }
int GetDevice() {
  int device;
...
size_t cublas_handle() {
  return (size_t)(Caffe::cublas_handle());
}
...
BOOST_PYTHON_MODULE(_caffe) {
  ...
  bp::def("get_device", &GetDevice);
  bp::def("set_random_seed", &Caffe::set_random_seed);
#ifndef CPU_ONLY
  bp::def("cuda_num_threads", &cuda_num_threads);
